...
const PharmaGuard = () => {
  ...
  const [selectedAnalysisJson, setSelectedAnalysisJson] = useState(null);
  const [showDrugJsonModal, setShowDrugJsonModal] = useState(false);
  ...
  const AnalysisResult = ({ analysis, patientId, isMulti }) => (
    ...
    >
      ...
      >
        ...
        <button
          onClick={() => {
            setSelectedAnalysisJson(analysis);
            setShowDrugJsonModal(true);
          }}
          style={{
            padding: "8px 14px",
            backgroundColor: "#ffffff",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://pharmaguard-gn0n.onrender.com/analyze",
        {
          method: "POST",
          body: formData,
        },
  ...
  return (
    ...
    >
      ...
      {showDrugJsonModal && (
        <DrugJsonModal
          analysis={selectedAnalysisJson}
          onClose={() => {
            setShowDrugJsonModal(false);
            setSelectedAnalysisJson(null);
          }}
        />
      )}
    </div>
